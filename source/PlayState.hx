...
class PlayState extends MusicBeatState
{
	...
	function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:Int = checkForAchievement([0, 1]);		// week status related achievements
			if(achieve > -1) {
	...
	private function checkForAchievement(arrayIDs:Array<Int>):Int {
		for (i in 0...arrayIDs.length) {
			if(!Achievements.achievementsUnlocked[arrayIDs[i]][1]) {
				switch(arrayIDs[i]) {
					case 0:		// week 1 clear 
						if(isStoryMode && storyPlaylist.length <= 1 && WeekData.getCurrentWeekNumber() == 1 && !usedPractice) {
							Achievements.unlockAchievement(arrayIDs[i]);
							return arrayIDs[i];
						}
					case 1:		// week 1 hard no miss
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'Hard' &&
						storyPlaylist.length <= 1 && WeekData.getCurrentWeekNumber() == 1 && !changedDifficulty && !usedPractice) {
							Achievements.unlockAchievement(arrayIDs[i]);
							return arrayIDs[i];
						}
				}
